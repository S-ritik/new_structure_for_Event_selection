...
void Anal_Leptop_PROOF::SlaveBegin(TTree * /*tree*/)
{
  ...
  for(int lvar=0; lvar<ncutflow; lvar++){
    ...
    sprintf(ltitlein,"inv mass of leptons after cut no %s",((string)to_string(lvar+1)).c_str());
    hist_met_cutflow[lvar] = new TH1D(lnamein,ltitlein,120,0,1000);
...
Bool_t Anal_Leptop_PROOF::Process(Long64_t entry)
{
  ...
  hist_met_cutflow[2]->Fill((vleptons[0].p4 + vleptons[1].p4).M(),weight);
...
#ifdef E_MU_TTBar
  is_additional_lepton = ( extra_mu >1 || extra_el >1);
...
#elif defined(E_E_TTBar)
  is_additional_lepton = ( extra_mu >0 || extra_el >2);
...
#elif defined(MU_MU_TTBar)
  is_additional_lepton = (extra_mu >2 || extra_el > 0);
  ...
  sel_cut.cut_pass = LJets.size()>=2 && vleptons.size()>=2 && delta2R(LJets[1].p4,vleptons[1].p4)<0.8;
  ...
  sel_cut.cut_pass = LJets.size()>=2 && !isnan(LJets[0].subhaddiff) && !isnan(LJets[1].subhaddiff);
  sel_cut.name = "Subjet hadronic energy fraction differences is not nan";
  event_cuts.push_back(sel_cut);
  histfill *= event_cuts[int(event_cuts.size()-1)].cut_pass;
  ...
  for(unsigned icut=0; icut<event_cuts.size(); icut++){
    ...
    if(event_pass){
      ...
      hist_met_cutflow[3+icut]->Fill((vleptons[0].p4 + vleptons[1].p4).M(),weight);
    }
  }
